...
if __name__ == '__main__':
    ...
    # if the last column for each row is bigger than 5 set value 1 else 0
    Y = np.where(Y_raw >= 5, 1, 0)
    
    split_idx = int(len(data) * (x / 100))
    train_X = X[split_idx:]
    train_Y = Y[split_idx:]
    ...
    least_import = np.argmin(importances)
    
    train_X_filt = np.delete(train_X,least_import, axis=1)
    test_X_filt = np.delete(test_X, least_import,axis=1)
    
    # Applied scaling
    scaler_std = StandardScaler()
    train_X_std = scaler_std.fit_transform(train_X_filt)
    test_X_std = scaler_std.transform(test_X_filt)
        
    mlp_std = MLPClassifier(hidden_layer_sizes=(15,), activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_std.fit(train_X_std, train_Y)
    
    pred_std = mlp_std.predict(test_X_std)
    acc_std = accuracy_score(test_Y, pred_std)
    
    
    # MinMaxScaler
    scaler_mm = MinMaxScaler()
    
    train_X_mm = scaler_mm.fit_transform(train_X_filt)
    test_X_mm = scaler_mm.transform(test_X_filt)
        
    mlp_mm = MLPClassifier(hidden_layer_sizes=(15,), activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_mm.fit(train_X_mm, train_Y)
    
    pred_mm = mlp_mm.predict(test_X_mm)
    acc_mm = accuracy_score(test_Y, pred_mm)
