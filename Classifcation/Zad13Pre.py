...
if __name__ == '__main__':
    ...
    # Za sekoja clasa treba 3 klasifikatori so odredeni podelbi megjkuk x1 x2
    for cls_data in classes:
        split_1 = int(len(cls_data) * x1)
        split_2 = int(len(cls_data) * x2)
        
        train_nb.extend(cls_data[:split_1])
        train_dt.extend(cls_data[split_1:split_2])
        train_rf.extend(cls_data[:split_2])
        
        test_data.extend(cls_data[split_2:])
    ...
    X_nb, Y_nb = get_xy(train_nb)
    X_dt, Y_dt = get_xy(train_dt)
    X_rf, Y_rf = get_xy(train_rf)
    ...
    # Koga imame ensable - eden zapis e tocen ako barem 2ca pogdile 
    # Acc = correct_guesses / length Y
    pred_nb = nb.predict(test_x)
    pred_dt = dt.predict(test_x)
    pred_rf = rf.predict(test_x)
    
    # Total correct od sekoja kolona na Y key
    correct_predictions = 0
    
    for i in range(len(test_y)):
        true_class = test_y[i]
        # Ova e za sekoja kolona kolku modeli tocno pogogile
        correct_votes = 0
        
        if pred_nb[i] == true_class:
            correct_votes += 1 
        if pred_dt[i] == true_class:
            correct_votes += 1
        if pred_rf[i] == true_class:
            correct_votes += 1
        
        if correct_votes >= 2:
            correct_predictions += 1
            
    total_acc = correct_predictions / len(test_y) if len(test_y) > 0 else 0.0
    print(f"Tochnost: {total_acc}")
