...
if __name__ == '__main__':
    ...
    class_0 = [row for row in dataset if int(row[-1]) == 0]
    class_1 = [row for row in dataset if int(row[-1]) == 1]
    
    split_0 = int(len(class_0) * (N / 100))
    split_1 = int(len(class_1) * (N / 100))
    
    train_dataset = class_0[:split_0] + class_1[:split_1]
    test_dataset = class_0[split_0:] + class_1[split_1:]
    ...
    train_X, train_Y = get_xy(train_dataset)
    test_X, test_Y = get_xy(test_dataset)
    ...
    # Niza na site modeli
    models = [nb, dt, rf, mlp]
    # Niza za iminjata da mozhe da ivadime indexot so najgolem acc
    ...
    print(f"Najgolema tocnost ima klasifikatorot {best_model_name}")
    
    # BEST ACCURACY MODEL ENAMBLE
    vote_predictions = []
    
    # Gledaj gi site y coloni za testirane
    for i in range(len(test_Y)):
        # Pravime dictonary za 0 i 1 klasite da vidime kolkku glasovi ima za sekoj
        votes = {0:0, 1:0}
        
        # Geldame sekoj model sto predvidel
        for m_idx in range(4):
            predicted_class = int(predictions[m_idx][i])
            
            if m_idx == best_model_idx:
                # Za predvidenata klasa dodavame 2
                votes[predicted_class] += 2
            else:
    ...
    # Reccal score go proveruva kolku tocno klasi pogodivme spored Test Y od vote sistemot
    # example:
    # - test_Y (Reality): e.g., [1, 0, 1, 1, 0]
    # - vote_predictions (Your Model's Guesses): e.g., [1, 0, 0, 1, 1]
    # Zero devision ne dozvoluva da se deli so 0 ako imame 0 tocni pogodeni 
    odziv = recall_score(test_Y, vote_predictions, zero_division=0)
