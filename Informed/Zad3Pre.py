...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        x, y = state
        return (x, y) not in walls and 0 <= x < n and 0 <= y < n
    ...
    def h(self, node):
        ...
        # Disnace left on x axis // 3
        steps_x = math.ceil(abs(goal_x - x) / 3)
        # Disnace left on y axis // 3
        steps_y = math.ceil(abs(goal_y - y) / 3)
