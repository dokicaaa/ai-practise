...
class Squares(Problem):
    ...
    def h(self, node):
        # Menehten rastorijanie na sekoe kvadratche
        state = node.state
        total_distance = 0

        for i, (x, y) in enumerate(state):
            # The index show which row it needs to be
            goal_x = i
            # For the y value we need it to be 1 less squre then the index
            # Expamle for to cordinate will be x = 1 , y = 3
            goal_y = 4 - i
