...
class Box(Problem):
    ...
    def check_valid(state):
        ...
        for x, y in [man, box1, box2]:
            if x < 0 or x >= 5 or y < 0 or y >= 5:
                return False

        if box1 == box2 and box1 != (4, 4):
            return False
        if man == box1 or man == box2:
            return False
    ...
    def successor(self, state):
        ...
        for direction_name, (dx, dy) in directions.items():
            new_man_x = man_x + dx
            new_man_y = man_y + dy
            new_man_pos = (new_man_x, new_man_y)

            if new_man_pos == box1:
                bx, by = box1
                new_box1_pos = (bx + dx, by + dy)
                new_state = (new_man_pos, new_box1_pos, box2)
                if self.check_valid(new_state):
                    succ[f"Push box 1 {direction_name}"] = new_state

            elif new_man_pos == box2:
                bx, by = box2
                new_box2_pos = (bx + dx, by + dy)
                new_state = (new_man_pos, box1, new_box2_pos)
                if self.check_valid(new_state):
                    succ[f"Push box 2 {direction_name}"] = new_state
            else:
                new_state = (new_man_pos, box1, box2)
                if self.check_valid(new_state):
...
if __name__ == '__main__':
    ...
    box1_pos = tuple(map(int, input().split(',')))
    box2_pos = tuple(map(int, input().split(',')))

    initial_state = (man_pos, box1_pos, box2_pos)

    box_problem = Box(initial_state, (4, 4))

    result = breadth_first_graph_search(box_problem)
