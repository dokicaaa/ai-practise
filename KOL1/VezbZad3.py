...
class BallsProblem(Problem):
    ...
    def goal_test(self, state):
        ...
        if len(state) != 1:
            return False
        # The remaining ball is at the goal position
    ...
    def successor(self, state):
        ...
        for ball in state:
            ...
            for direction, (dx, dy) in directions.items():
                # Calculate the position of the jumped-over ball (midpoint)
                mid_x = x + dx // 2
                mid_y = y + dy // 2
                ...
                # 1. Landing position is valid and empty
                # 2. There's a ball to jump over at the midpoint
                if self.check_valid(new_x, new_y) and (new_x, new_y) not in state and (mid_x, mid_y) in state:
                    # Create new state
                    new_balls = list(state)
                    new_balls.remove(ball)  # Remove the moving ball from old position
                    new_balls.remove((mid_x, mid_y))  # Remove the jumped-over ball
                    new_balls.append((new_x, new_y))  # Add the moving ball to new position

                    new_state = tuple(sorted(new_balls))
                    succ[f"{direction}: (x={x},y={y})"] = new_state
...
if __name__ == "__main__":
    ...
    for _ in range(num_balls):
        initial_balls.append(tuple(map(int, input().split(','))))
    ...
    problem = BallsProblem(tuple(sorted(initial_balls)), n, obstacles)
