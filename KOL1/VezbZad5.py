...
# state x , y position na pacaman , direction , i tuple sorted remainng dots
#najaml broj na potezi
...
move_offsets = {
    'north': {'forward': (0, 1), 'backward': (0, -1), 'left': (-1, 0), 'right': (1, 0)},
    'east':  {'forward': (1, 0), 'backward': (-1, 0), 'left': (0, 1), 'right': (0, -1)},
    'south': {'forward': (0, -1), 'backward': (0, 1), 'left': (1, 0), 'right': (-1, 0)},
    'west':  {'forward': (-1, 0), 'backward': (1, 0), 'left': (0, -1), 'right': (0, 1)},
}
...
class Packman(Problem):
    ...
    def successor(self, state):
        successors = {}
        x, y, direction, dots = state
        index = facing.index(direction)
        offsets = move_offsets[direction]
        ...
        for action, (dx, dy) in offsets.items():
            ...
            if (new_x, new_y) in dots:
                new_dots = list(dots)
                new_dots.remove((new_x, new_y))
                new_dots = tuple(sorted(new_dots))
            else:
                new_dots = dots

            successors[name] = (new_x, new_y, new_dir, new_dots)
...
if __name__ == "__main__":
    ...
    initial_state = (pac_x, pac_y, direction, tuple(sorted(dots)))
    result = breadth_first_graph_search(Packman(initial_state))
