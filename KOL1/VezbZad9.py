    ...
    def successor(self, state):
        ...
        for i in range(num_pillars):
            ...
            for j in range(num_pillars):
                ...
                if not state[j] or state[j][0] > top_block:
                    new_state = list(list(pillar) for pillar in state)
                    new_state[i].pop(0)
                    new_state[j].insert(0, top_block)

                    new_state_tuple = tuple(tuple(p) for p in new_state)

                    action = f"MOVE TOP BLOCK FROM PILLAR {i+1} TO PILLAR {j+1}"
                    succ[action] = new_state_tuple
    ...
    def parse_state(line):
        ...
        for part in line.split(';'):
            ...
            for block in part.split(','):
                if block.strip():
                    pillar.append(int(block))
            state.append(tuple(pillar))
        return tuple(state)

    initial = parse_state(line1)
    goal = parse_state(line2)

    pillar_problem = PillarProblem(initial, goal)

    result = breadth_first_graph_search(pillar_problem)

    if result:
        print(f"Number of action {result.path_cost}")
