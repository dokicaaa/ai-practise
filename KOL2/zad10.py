...
if __name__ == '__main__':
    ...
    # Least imporatnt go zemam zo np.argmin, kade go pasnuvash feature imporatneces na modelot
    least_important_idx = np.argmin(model1.feature_importances_)

    train_X_filtered = np.delete(train_X, least_important_idx, axis=1)
    test_X_filtered = np.delete(test_X, least_important_idx, axis=1)

    # SKALRINAJE I NEVROSNKA MREZHA
    scaler_std = StandardScaler()
    train_X_std = scaler_std.fit_transform(train_X_filtered)
    test_X_std = scaler_std.transform(test_X_filtered)  # transform only!

    mlp_std = MLPClassifier(hidden_layer_sizes=(15,), activation='relu',
                            learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_std.fit(train_X_std, train_Y)
    acc_std = accuracy_score(test_Y, mlp_std.predict(test_X_std))

    # MinMaxScaler
    scaler_mm = MinMaxScaler()
    train_X_mm = scaler_mm.fit_transform(train_X_filtered)
    test_X_mm = scaler_mm.transform(test_X_filtered)  # transform only!

    mlp_mm = MLPClassifier(hidden_layer_sizes=(15,), activation='relu',
                           learning_rate_init=0.001, max_iter=200, random_state=0)
    mlp_mm.fit(train_X_mm, train_Y)
    acc_mm = accuracy_score(test_Y, mlp_mm.predict(test_X_mm))
