...
if __name__ == '__main__':
    hidden_neurons = int(input().strip())
    learning_rate = float(input().strip())
    col_to_delete = int(input().strip())

    # Go stavame input vo niza i gi kastirame vo float
    input_line = input().split()
    new_input = np.array([float(x) for x in input_line])
    ...
    if (acc_train - acc_val) >= 0.15:
        ...
        X_train_filtered = np.delete(X_train, col_to_delete, axis=1)
        sample_filtered = np.delete(new_input, col_to_delete)

        scaler_new = MinMaxScaler(feature_range=(-1, 1))
        X_train_f_s = scaler_new.fit_transform(X_train_filtered)

        sample_scaled = scaler_new.transform([sample_filtered])
        model_new = MLPClassifier(hidden_layer_sizes=(hidden_neurons,),
                                  activation='relu',
                                  learning_rate_init=learning_rate,
                                  max_iter=1000,)
        model_new.fit(X_train_f_s, Y_train)

        prediction = model_new.predict(sample_scaled)
        print(int(prediction[0]))

    else:
        print(f"Ne se sluchuva overfitting")

        sample_scaled = scaler.transform([new_input])
