...
if __name__ == '__main__':
    ...
    # od start od x1 za sekoja klasa
    def get_splits(class_data, p1, p2):
        idx1 = int(len(class_data) * p1)
        idx2 = int(len(class_data) * p2)

        train_nb = class_data[:idx1]

        train_dt = class_data[idx1: idx2]

        train_rf = class_data[:idx2]

        test_data = class_data[idx2:]

        return train_nb, train_dt, train_rf, test_data


    nb_0, dt_0, rf_0, test_0 = get_splits(class0, x1, x2)
    nb_1, dt_1, rf_1, test_1 = get_splits(class1, x1, x2)
    nb_2, dt_2, rf_2, test_2 = get_splits(class2, x1, x2)

    # odkoga podeluvash morash da spoish dataset za sekoj vid drvo vo 3 glavni datasets
    train_NB = nb_0 + nb_1 + nb_2
    train_DT = dt_0 + dt_1 + dt_2
    train_RF = rf_0 + rf_1 + rf_2
    test_dataset = test_0 + test_1 + test_2


    # Sega otkatko sme gi spoile klasite treba da gi zememe tipicni labelot i atrivutite
    ...
    rf_model = RandomForestClassifier(n_estimators=3,criterion='entropy', random_state=0,)

    X_nb, Y_nb = split_data(train_NB)
    X_dt, Y_dt = split_data(train_DT)
    X_rf, Y_rf = split_data(train_RF)
    ...
    # VOTING SYSTEM - THIS IS NEW. WE NEED TO SEE IF ONE OUTCOME HAS BEEN GUESSED MOULTPLE TIME BY AT LEASTG TWO MODELs
    ...
    for i in range(len(Y_test)):
        ...
        if pred_nb[i] == true_label:
            votes_for_truth += 1
        if pred_dt[i] == true_label:
            votes_for_truth += 1
        if pred_rf[i] == true_label:
            votes_for_truth += 1

        if votes_for_truth >= 2:
