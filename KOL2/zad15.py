...
if __name__ == '__main__':
    split_X = int(input().strip())

    class0 = [row for row in dataset if int(row[-1]) == 0]
    class1 = [row for row in dataset if int(row[-1]) == 1]
    idx0 = int(len(class0) * (split_X / 100.0))
    idx1 = int(len(class1) * (split_X / 100.0))

    train_data = class0[:idx0] + class1[:idx1]
    test_data = class0[idx0:] + class1[idx1:]

    def get_xy(data):
        # Кастирање на атрибутите во float, а класата во int
    ...
    model3 = RandomForestClassifier(n_estimators=4, criterion='entropy', random_state=0)
    ...
    for i in range(len(Y_test)):
        ...
        for m_idx in range(4):
            pred_class = int(all_predictions[m_idx][i])
    ...
    odziv = recall_score(Y_test, vote_predictions, zero_division=0)
