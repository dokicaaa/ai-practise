...
class Snake(Problem):
    def __init__(self, initial, red_apples):
        ...
        # Gi zacuvuvame kako tuple od kordinati
        self.red_apples = tuple(red_apples)

    def check_valid(self,state):
        body, direction, g_apples = state
        head_x, head_y = body[0]

        # Check if in bounds of board
        if not(0 <= head_x < 10 and 0 <= head_y < 10):
            return False

        # Check if the head hits red apples
        if (head_x, head_y) in self.red_apples:
            return False

        #Check if the snakes touches its own body
        if (head_x, head_y) in body[1:]:
            return False
    ...
    def successor(self, state):
        ...
        # Define the mooving direction - We must take in consideration the
        # 4 faces S N W E each time we make a move
        directions = {
        ...
        for action, new_dir in directions.items():
            dx, dy = offsets[new_dir]
            new_head = (head_x + dx, head_y + dy)

            # Need to calculate new body and take out one green apple
            new_g_apples = list(g_apples)
            if new_head in new_g_apples:
                new_g_apples.remove(new_head)
                new_body = (new_head,) + body
            else:
            # Ako ne sme dobile jaolkko go trgmae najposldnata koridnat za opaskata
...
if __name__ == '__main__':
    # 1. Read the number of Red Apples
    n_red = int(input().strip())
    red_apples = []
    for _ in range(n_red):
        # This splits '6,9' into '6' and '9', then maps them to integers
        x, y = map(int, input().strip().split(','))
        red_apples.append((x, y))

    # 2. Read the number of Green Apples
    n_green = int(input().strip())
    green_apples = []
    for _ in range(n_green):
        # Same here for the green apples
        x, y = map(int, input().strip().split(','))
        green_apples.append((x, y))

    initial_state = (((0, 2), (0, 1), (0, 0)), 'S', tuple(green_apples))

    problem = Snake(initial_state, red_apples)
