...
# State: 1 d arrat tuple, with binary values with Lengh N^2. For each cell in the NxN grid
...
class Board(Problem):
    ...
    def successor(self, state):
        ...
        # Try clicking every cell on the NxN board
        for r in range(self.n):
            for c in range(self.n):
                # define the action that neesd to be added in succ
                action = f'x: {r}, y: {c}'

                new_state = list(state)

                # Define that the cell was clicked and its up down right left neighbors
                flips = [(r,c)]
                if r > 0:
                    flips.append((r - 1, c)) #UP
                if r < self.n -1:
                    flips.append((r+1, c)) #DOWN
                if c > 0:
                    flips.append((r, c - 1))
                if c < self.n - 1:
                    flips.append((r, c + 1))
                ...
                #Chabnge colors to clicked cell and neigbhbour cells
                # First convert 2d r,c in 1d index
    ...
    def goal_test(self, state):
        # When every signle value in the tuple is 1
        # This can be done if the sum of each idk to be == NxN
        return sum(state) == self.n * self.n


if __name__ == "__main__":
    n = int(input().strip())
    fields_input = input().strip().split(',')


    initial_state = tuple(int(x) for x in fields_input)

    problem = Board(initial_state, n)
