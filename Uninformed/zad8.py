...
# State (1, 2 , 3 , 0, 0 , 0). Kade golemina na nizasta ke bide L

class Disks(Problem):
    def __init__(self, initial, goal, L):
    ...
    def successor(self, state):
        ...
        # Ittrate through each index in the arr
        for i in range(self.L):
            # check if there is a disk on this spot
            if state[i] != 0:
                disk = state[i]
                
                # Try R1 - > must be withing bounds and must next slot be empty
                if i + 1 < self.L and state[i + 1] == 0:
                    new_state = list(state)
                    new_state[i], new_state[i+1] = new_state[i+1], new_state[i]
                    succ[f'R1: Disk {disk}'] = tuple(new_state)
                
                # Try R2 -> must be within bounds jump over a disk, int tempy cell 
                ...
                # Try L1 -> must be in left bound
                if i - 1 >= 0 and state[i - 1] == 0:
                    new_state = list(state)
                    new_state[i], new_state[i-1] = new_state[i-1], new_state[i]
                    succ[f'L1: Disk {disk}'] = tuple(new_state)
                ...
                if i - 2 >= 0 and state[i-1] != 0 and state[i-2] == 0:
                    new_state = list(state)
                    new_state[i], new_state[i-2] = new_state[i-2], new_state[i]
                    succ[f'L2: Disk {disk}'] = tuple(new_state)
...
if __name__ == "__main__":
    ...
    # e.g., for N=3, L=7: [1, 2, 3] + [0, 0, 0, 0] -> (1, 2, 3, 0, 0, 0, 0)
    initial_list = list(range(1, N + 1)) + [0] * (L - N)
    initial_state = tuple(initial_list)
    
    # 3. Construct the Goal State
    # e.g., for N=3, L=7: [0, 0, 0, 0] + [3, 2, 1] -> (0, 0, 0, 0, 3, 2, 1)
    goal_list = [0] * (L - N) + list(range(N, 0, -1))
    goal_state = tuple(goal_list)
    
    problem = Disks(initial_state, goal_state, L)
